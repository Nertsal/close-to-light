...
vec4 dither_final(vec2 amps) {
	...
	float noise_accent = 0.1 * (noise(vec3(u_time * 16.0, (get_pixel_pos() + vec2(1000.0)) * 2.0)) * 2.0 - 1.0);
	
	float amp_light = amps.g + noise_light;
	float amp_accent = amps.r + noise_accent;
	float total = amp_light + amp_accent;
	if (total > 1.0) {
		amp_light /= total;
		amp_accent /= total;
	}
	
	if (dither_inverted(amp_accent) > 0.0) return u_color_accent;
