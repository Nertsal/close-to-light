    ...
    pub fn update(&mut self, mut main: Aabb2<f32>, state: &mut MenuState, context: &mut UiContext) {
        ...
        if let Some(show) = &state.selected_music {
            if let Some(music) = local.music.get(&show.data) {
                ...
                self.music.update(music_pos, context);
                self.music.options.size = music_pos.height() * font_factor;
                self.music_author.update(music_author_pos, context);
            ...
            .and_then(|group| local.groups.get(group.data))
        {
            if let Some(show) = &state.selected_level {
                if let Some(level) = group.levels.get(show.data) {
                    ...
                    self.difficulty.update(difficulty_pos, context);
                    self.difficulty.options.size = difficulty_pos.height() * font_factor;
                    self.mappers.update(mappers_pos, context);
